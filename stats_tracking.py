...
class Tracker:
    ...
    def __init__(self, model, track_logits=False, device='cpu', requires_backward=True):
    ...
    def stop_tracking(self):
        for hook in list(self._f_hooks.values()) + list(self._b_hooks.values()):
            hook.remove()

    def resume_tracking(self):
        for name, module in self.modules.item():
            self._f_hooks[name] = module.register_forward_hook(self._f_hook)
...
class ActivationTracker(Tracker):
    ...
    def __init__(self, module, *args, **kwargs):
        super(ActivationTracker, self).__init__(module, *args, **kwargs)
        self.last_out = {}
